...
public class ScanbotCameraPlugin extends ScanbotCordovaPluginBase {
    ...
    private static final int REQ_CODE_BARCODE_SCANNER_PERMISSIONS = 4713;
    ...
    private final static String[] REQUIRED_BARCODE_SCANNER_PERMISSIONS = {
            Manifest.permission.CAMERA,
    ...
    public boolean execute(final String action, final JSONArray args, final CallbackContext callbackContext) throws JSONException {
        ...
        if (action.equals("startBarcodeScanner")) {
            checkPermissionsAndStartBarcodeScanner();
            return true;
        }

        if (action.equals("dismissBarcodeScanner")) {
            final Intent closeIntent = new Intent(ACTION_DISMISS_SB_BARCODE_SCANNER);
            this.cordova.getActivity().sendBroadcast(closeIntent);
            return true;
        }
    ...
    public void onActivityResult(final int requestCode, final int resultCode, final Intent intent) {
        ...
        if (requestCode != REQUEST_SB_CAMERA &&
                requestCode != REQUEST_SB_EDIT_IMAGE &&
                requestCode != REQUEST_SB_BARCODE_SCANNER) {
            return;
        }
        ...
        switch (resultCode) {
            ...
            case RESULT_SB_EDIT_IMAGE:
                ...
                return;
            case RESULT_SB_EDIT_IMAGE_ERROR:
                this.handleActivityErrorResult(intent);
                return;
            case RESULT_SB_BARCODE_SCANNER:
                this.handleBarcodeScannerResult(intent);
                return;
            default:
                return;
        }
    ...
    /**
     * Checks/requests camera permissions and starts Scanbot Barcode Scanner UI.
     *
     * @throws JSONException
     */
    private void checkPermissionsAndStartBarcodeScanner() throws JSONException {
        // check required permissions
        for (final String p: REQUIRED_BARCODE_SCANNER_PERMISSIONS) {
            if (!this.cordova.hasPermission(p)) {
                this.cordova.requestPermissions(this,
                        REQ_CODE_BARCODE_SCANNER_PERMISSIONS, REQUIRED_BARCODE_SCANNER_PERMISSIONS);
                return;
            }
        }

        startScanbotBarcodeScannerActivity();
    }
    ...
    public void onRequestPermissionResult(final int requestCode, final String[] permissions, final int[] grantResults) throws JSONException {
        ...
        switch (requestCode) {
            ...
            case REQ_CODE_EDIT_IMG_PERMISSIONS:
                startScanbotEditImageActivity();
                break;
            case REQ_CODE_BARCODE_SCANNER_PERMISSIONS:
                startScanbotBarcodeScannerActivity();
                break;
            default:
                return;
     ...
     * @throws JSONException
     */
    private void startScanbotBarcodeScannerActivity() throws JSONException {
        final boolean flashEnabled = getJsonArg(jsonArgs, "flashEnabled", true);
        final boolean playTone = getJsonArg(jsonArgs, "playTone", true);
        final boolean vibrate = getJsonArg(jsonArgs, "vibrate", true);
        final List<String> barcodeFormats = getJsonArg(jsonArgs, "barcodeFormats");
        debugLog("flashEnabled: " + flashEnabled);
        debugLog("playTone: " + playTone);
        debugLog("vibrate: " + vibrate);
        debugLog("barcodeFormats: " + barcodeFormats);

        final Intent intent = new Intent(this.cordova.getActivity(), ScanbotBarcodeScannerActivity.class);
        final Bundle extras = new Bundle();
        extras.putBoolean(EXTRAS_ARG_FLASH_ENABLED, flashEnabled);
        extras.putBoolean(EXTRAS_ARG_PLAY_TONE, playTone);
        extras.putBoolean(EXTRAS_ARG_VIBRATE, vibrate);
        extras.putStringArrayList(EXTRAS_ARG_BARCODE_FORMATS, new ArrayList<String>(barcodeFormats));
        intent.putExtras(extras);
    ...
    private void handleCamPictureTakenResult(final Intent intent) {
        ...
        final String detectionResult = intent.getStringExtra(EXTRAS_ARG_DETECTION_RESULT);
        ...
        debugLog( "Got polygon from Scanbot Camera UI Activity: " + polygon);
        debugLog( "Got detection result from Scanbot Camera UI Activity: " + detectionResult);
        final JSONObject result = new JsonArgs()
                ...
                .put("detectionResult", detectionResult)
                .jsonObj();
    ...
    private void handleBarcodeScannerResult(final Intent intent) {
        final String jsonResult = intent.getStringExtra(EXTRAS_ARG_BARCODE_JSON_RESULT);
        debugLog( "Got JSON string result from BarcodeScanner Activity: " + jsonResult);
        try {
            this.callbackContext.success(new JSONObject(jsonResult));
        } catch (final JSONException e) {
            final String errorMsg = "Could not build JSON result callback. " + e.getMessage();
            errorLog(errorMsg, e);
    ...
    private void handleActivityErrorResult(final Intent intent) {
        final String errorMsg = intent.getStringExtra(EXTRAS_ARG_ERROR_MSG);
        errorLog(errorMsg);
        this.callbackContext.error(errorMsg);
    }
