...
public class ScanbotSdkPlugin extends ScanbotCordovaPluginBase {
    ...
    public boolean execute(final String action, final JSONArray args, final CallbackContext callbackContext) throws JSONException {
        ...
        if (action.equals("isLicenseValid")) {
            try {
                ...
                final String errorMsg = "Could not perform license validation: " + e.getMessage();
                errorLog(errorMsg, e);
                callbackContext.error(errorMsg);
            }
            return true;
        ...
        if (action.equals("rotateImage")) {
            try {
                ...
                final String errorMsg = "Could not apply image rotation: " + e.getMessage();
                errorLog(errorMsg, e);
                callbackContext.error(errorMsg);
            }
            return true;
    ...
    private void isLicenseValid(final CallbackContext callbackContext) throws JSONException {
        debugLog("Validating Scanbot SDK license...");
        cordova.getActivity().runOnUiThread(new Runnable() {
            public void run() {
                    ...
                    public void run() {
                        try {
                            callbackContext.success(new JsonArgs()
                                    .put("isLicenseValid", scanbotSDK.isLicenseActive())
                                    .jsonObj());
                        } catch (final Exception e) {
                            final String errorMsg = "Error validating Scanbot SDK license: " + e.getMessage();
                            errorLog(errorMsg, e);
                            callbackContext.error(errorMsg);
                        }
                    }
        ...
        cordova.getThreadPool().execute(new Runnable() {
            public void run() {
                try {
                    ...
                    final String errorMsg = "Error applying rotation on image: " + imageFileUri;
                    errorLog(errorMsg, e);
                    callbackContext.error(errorMsg);
                }
            }
