        ...
        if (result) {
            pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK
                                         messageAsDictionary:@{@"imageFileUri":imageUrl.absoluteString,
                                                               ...
                                                               @"polygon":(self.detectedPolygon ? [self.detectedPolygon polygonPoints] : [NSMutableArray new]),
                                                               @"detectionResult":(self.detectedPolygon ? [self.detectedPolygon detectionStatusString] : @"ERROR_NOTHING_DETECTED")
                                                               }];
        } else {
...
- (void)scannerController:(SBSDKScannerViewController *)controller
          didCaptureImage:(nullable UIImage *)image
...
localizedTextForDetectionStatus:(SBSDKDocumentDetectionStatus)status {
    ...
    switch (status) {
        ...
        case SBSDKDocumentDetectionStatusOK_BadAspectRatio:
            return localizedStatuses[@"autosnapping_hint_bad_aspect_ratio"];
