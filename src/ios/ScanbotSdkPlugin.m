...
- (void)isLicenseValid:(CDVInvokedUrlCommand *)command {
    ...
    pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK
                                 messageAsDictionary:@{@"isLicenseValid":@([ScanbotSDK isLicenseValid])}];
#ifdef _CORDOVA_TEST_TARGET
	...
	[self.commandDelegate runInBackground:^{
		...
		if (inputImage) {
			...
			if ([ImageUtils saveImage:outputImageFilePath
							  ...
							  quality:quality]) {
				pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK
											 messageAsDictionary:@{@"imageFileUri":[NSURL fileURLWithPath:outputImageFilePath].absoluteString}];
			} else {
			...
			pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR
											 messageAsString:@"Input image not found"];
	#ifdef _CORDOVA_TEST_TARGET
			command.testCaseCompletionBlock(pluginResult);
	#else
			[self.commandDelegate sendPluginResult:pluginResult
